...
namespace VuelosApp.DAL.BulkOperation
{
    public class Operation
    {
        DatabaseFacade _database;
        string _connectString;

        public Operation(DatabaseFacade database)
        {
            this._database = database;
            this._connectString = _database.GetDbConnection().ConnectionString;
        }
        ...
        public void BulkInsert<T>(IEnumerable<T> list, string parentTableName) where T : class
        {
            using (SqlConnection conn = new SqlConnection(_connectString))
            {
                ...
                {
                    using (var bulkCopy = new SqlBulkCopy(_connectString, SqlBulkCopyOptions.Default) { DestinationTableName = parentTableName })
                    {
                        using (var childBulkCopy = new SqlBulkCopy(_connectString, SqlBulkCopyOptions.Default))
                        {
